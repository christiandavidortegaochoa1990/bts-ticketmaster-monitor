...
def save_state(current_hash, event_count, last_check_summary, last_heartbeat_sent=None):
    ...
    # Only update heartbeat if explicitly provided
...
def check_for_changes():
    ...
    if len(events) == 0 and previous_count > 0:
        ...
        result["message"] = "⚠️ BTS events are no longer available on Ticketmaster!"
        ...
        result["message"] = f"🎫 NEW EVENT: {event_name}\n📅 Date: {event_date}"
        ...
        if start_date_sales != "N/A":
            result["message"] = f"🔔 TICKET UPDATE: {event_name}\n💫 Sales started: {start_date_sales}"
        else:
            result["message"] = f"🔔 EVENT UPDATED: {event_name}"
...
def run_check_once():
    """Run one check and exit (main mode for Task Scheduler)."""
    print(f"\n{'='*60}")
    ...
    # Primary: Send change alert if detected
    ...
    # Secondary: Send heartbeat if due and no main alert was sent
    ...
    # Save state
    save_state(calculate_hash(search_events()[0]), result["events_found"], result, 
...
Examples:
  ...
  python main.py --check-once         # Run one check (for Task Scheduler)
